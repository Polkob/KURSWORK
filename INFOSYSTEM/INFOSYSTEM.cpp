...
void MusicCollection::AddTrack()
{
	...
	while (true)
	{
		...
		{
			//cin.clear();
			if (!(cin >> tmp->listening) || tmp->listening < 0)
				throw exception("Повторите ввод.");
			break;
		}
		catch (exception& ex)
		{
	...
	while (true)
	{
		try
		{
			if (!(cin >> tmp->listeningBelarus) || tmp->listeningBelarus < 0)
				throw exception("Повторите ввод.");
			break;
		}
		catch (exception& ex)
...
void MusicCollection::Edit() {
	...
	while(InputFlag=1)
	{
			...
			int answer;
			cout << "\nХотите изменить еще что-нибудь?(1 - да / 2 - нет): ";
		...
		int answ;
		cout << "\nХотите отменить изменения?(1 - да/2 - нет): ";
		cin >> answ;
...
void MusicCollection::GenreSelection()
{
	while (true)
	{
		try
		{
			...
			cout << "Введите жанр(pop/phonk/indie): "<<endl;
			/*cin.clear();
			cin.ignore();*/
			 ...
			 }if (check == 0)cout << "Треков с таким жанром нет." << endl;

			 check = 0;
			cout << "\nВы хотите просмотреть только популярные треки? (1 - да/ 2 - нет) : " << endl;
			if (!(cin >> otvet))
				throw exception("Неверный ввод, повторите попытку!");
			//cin >> otvet;
			...
			if (otvet == 1)
			{
				for(AllTracks* f : musics)
				{
					...
					{
						cout << "\t\t\t\t\\t<Популярные треки с жанром"<<temp<<">" << endl;
						f->Print();
						check++;
					}
				}
			...
			int answ;
			int flag = 0;
			cout << "\nХотите просмотреть рекомендации для этого жанра? (1 - да/ 2 - нет) : \n\n";
			...
			if (answ == 1)
			{
				...
				cout << "\t\t\t\t\t<Рекомендации по жанру>\n" << endl;
					...
					if (rec->genre == temp)
					{
						rec->Print();
						flag++;
					}if (flag == 0)cout << "В рекомендациях данный трек не находится." << endl;
...
void MusicCollection::EditPlaylist()
{
	...
	if (temp == 1)
	{
		...
		int index;
		int flagg=0;
		cout << "\nВведите номер трека для удаления: ";
		while (true)
		{
			try {
	...
	else if (temp == 2)
	{
		...
		int flag = 0;
		int povtor = 0;
		cout << "\nВведите номер трека для добавления: ";
		while (true)
		{
			try
			{
...
{
	cout << "\n" << setw(57)<<right << "<Menu>\t\n";
	cout << setw(37) << right << " " << "_________________________________\n";
	cout << setw(40) << right << "1|" << " Просмотреть список треков.\n";
	cout << setw(40) << right << "2|" << " Добавить трек.\n";
	cout << setw(40) << right << "3|" << " Удалить трек.\n";
	cout << setw(40) << right << "4|" << " Рекомендации.\n";
	cout << setw(40) << right << "5|" << " Топ треков во всем мире.\n";
	cout << setw(40) << right << "6|" << " Топ треков по жанру.\n";
	cout << setw(40) << right << "7|" << " Топ треков в Беларуси.\n";
	cout << setw(40) << right << "8|" << " Найти трек.\n";
	cout << setw(40) << right << "9|" << " Сортировка.\n";
	cout << setw(40) << right << "10|" << " Просмотреть плейлист.\n";
	cout << setw(40) << right << "11|" << " Изменить плейлист.\n";
	cout << setw(40) << right << "12|" << " Изменить трек.\n";
	cout << setw(40) << right << "13|" << " Подобрать трек по жанру.\n";
	cout << setw(40) << right << "14|" << " Выход.\n";
	cout << setw(37) << right << " " << "_________________________________\n";
